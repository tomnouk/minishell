...
static void copy_assign(char *string, t_data *data, t_tkn_data *token, t_list *node,int *start, int *current, t_types type)
{
...
void ft_word_token(t_data *data, int *start, int *current, t_types type)
{
	...
	if (!(quote_flag % 2))
		...
		copy_assign(string, data, token, node, start, current, type);
	if (*current == '\n')
		(*current)++;
}
